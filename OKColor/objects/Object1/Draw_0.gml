...
if (!surface_exists(surf1)) {
    ...
    for (var i = 0; i < 360; i++) {
        ...
        var color = color1.cloneMixed(color3, i / 360, OKColorMixing.RGB).color();
...
if (!surface_exists(surf2)) {
    ...
    //     draw_set_color(make_color_rgb(rgb.r, rgb.g, rgb.b));
        var color = color1.cloneMixed(color3, i / 360, OKColorMixing.Lab).color();
        draw_set_color(color);
        draw_rectangle(i * 2, 0, i * 2 + 1, 80, false);
    }
    surface_reset_target();
...
if (!surface_exists(surf3)) {
    ...
    //     draw_set_color(make_color_rgb(rgb.r, rgb.g, rgb.b));
    //     draw_rectangle(i * 2, 0, i * 2 + 1, 80, false);
        var color = color1.cloneMixed(color3, i / 360, OKColorMixing.OKLab).color();
        draw_set_color(color);
        draw_rectangle(i * 2, 0, i * 2 + 1, 80, false);
    }
    surface_reset_target();
